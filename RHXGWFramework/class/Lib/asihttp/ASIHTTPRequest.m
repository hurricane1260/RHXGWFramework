...
// 	if ([self proxyHost] && [self proxyPort]) {
    if (![self validatesSecureCertificate] && [self proxyHost] && [self proxyPort]) {
		NSString *hostKey;
		NSString *portKey;
...
- (void)runPACScript:(NSString *)script
{
	if (script) {
		...
		CFRelease(CFNetworkCopyProxiesForURL((CFURLRef)[self url], (CFDictionaryRef)tempDic));
	...
	@synchronized (self) {
		...
		if (!defaultUserAgent) {
			...
			if (marketingVersionNumber && developmentVersionNumber) {
				...
				appVersion = (marketingVersionNumber ? : developmentVersionNumber);
			}
