    ...
    if (IDInfo.type == 1) {
        ...
        if(GET_FULLIMAGE){
            if (frontFullImageView != nil)
                [frontFullImageView setImage:IDInfo.fullImg];
        }
    }else{
        //签发机关
        ...
        if(GET_FULLIMAGE){
            if (backFullImageView != nil)
                [backFullImageView setImage:IDInfo.fullImg];
        }
    }
    
    //是否显示整幅身份证图像
//    if(GET_FULLIMAGE){
//        UIImageView * fullImageView = [[UIImageView alloc] initWithFrame:CGRectMake(5, validValueTextField.frame.origin.y
//                                                                                    + validValueTextField.frame.size.height + 5, SCREEN_WIDTH - 10, (SCREEN_WIDTH - 10) * 0.632)];
//        [fullImageView setImage:IDInfo.fullImg];
//        [validValueTextField.superview addSubview:fullImageView];
//    }
...
{
    [delegate didBackfromIDRst:self];
}
