...
-(void)doRec:(UIImage *)image
{
    @synchronized(self) {
        ...
        if(nStatus >= 0)
        {
            ...
            //normal info
            idInfo.type = idcard.nType;
            if(idcard.nType == 1 ){
                idInfo.name = [NSString stringWithCString:idcard.szName encoding:gbkEncoding];
                idInfo.gender = [NSString stringWithCString:idcard.szSex encoding:gbkEncoding];
                idInfo.nation = [NSString stringWithCString:idcard.szNation encoding:gbkEncoding];
                idInfo.code = [NSString stringWithCString:idcard.szCardID encoding:gbkEncoding];
                idInfo.address = [NSString stringWithCString:idcard.szAddress encoding:gbkEncoding];
            } else if (idcard.nType == 2 ){
                idInfo.issue = [NSString stringWithCString:idcard.szIssue encoding:gbkEncoding];
                idInfo.valid = [NSString stringWithCString:idcard.szValid encoding:gbkEncoding];
            }
            //convert image
            ...
            if(pbImage){
                ...
                if(GET_FULLIMAGE){
                    idInfo.fullImg = [UIImage imageWithCGImage:quartzImage];
                }
        ...
        if (idInfo !=nil && [idInfo isOK]) {
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [self.delegate returnIDPhotoResult:idInfo from:self];
            });
        } else {
            dispatch_async(dispatch_get_main_queue(), ^{
                [self.indicator stopAnimating];
                ...
                self.backgroundView.hidden = YES;
                target.view.userInteractionEnabled = YES;
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"无法识别该图片，请重新识别" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil];
                [alert show];
            });
        }
#endif
