...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    if (abandonFrameCount++ < ABANDON_FRAME_COUNT) {        //abandon first ABANDON_FRAME_COUNT frames
        NSLog(@"abandon %d frame", abandonFrameCount);
        return;
    }
    if (lFrameCount ++ % 5 == 0)
    {
    ...
    @synchronized(self)
    {
        ...
        if(CVPixelBufferLockBaseAddress(imageBuffer, 0) == kCVReturnSuccess)
        {
            ...
            if (sufficientFocus) {
                ...
                if (ret <= 0){   //failed
                    NSLog(@"BANKCardRecApi，ret[%d]", ret);
                }else{          //succeed
                    bHasResult = YES;
                    NSLog(@"ret=[%d]", ret);
                    NSLog(@"%f", focusScore);
                    ...
                    NSLog(@"%d/%d",bCard.expiryMonth, bCard.expiryYear);
                    ...
                    bankInfo.fullImg = [self getSubImage:rect inImage:image];
                    //save guideRect image
                    if (0) {
                        ...
                        dispatch_async(dispatch_get_main_queue(), ^{
                            if (FILTER_BANK) {
                                if ([self.recDelegate bankIsSupport:bankInfo.bankName]) {
                                    ...
                                    NSLog(@"支持，银行名称: %@", bankInfo.bankName);
                                    [self.recDelegate BankCardRecognited:bankInfo];
                                } else {
                                    NSLog(@"不支持，银行名称: %@", bankInfo.bankName);
