...
- (void)commonInitialization {
    ...
    if (NSFoundationVersionNumber > NSFoundationVersionNumber_iOS_6_1) {
        _unselectedTitleAttributes = @{
                                       NSFontAttributeName: font1_common_xgw,
                                       NSForegroundColorAttributeName: [UIColor blackColor],
                                       };
    } else {
#if __IPHONE_OS_VERSION_MIN_REQUIRED < __IPHONE_7_0
        _unselectedTitleAttributes = @{
                                       UITextAttributeFont: font1_common_xgw,
    ...
    _badgeTextFont = font1_common_xgw;
    _badgePositionAdjustment = UIOffsetZero;
}

@synthesize badgeValue = _badgeValue;
-(NSString *)badgeValue {
    if ([_badgeValue isKindOfClass:[NSNumber class]]) {
        _badgeValue = [NSString stringWithFormat:@"%@",_badgeValue];
    }
    return _badgeValue;
}
...
- (void)drawRect:(CGRect)rect {
    ...
    if ([[self badgeValue] integerValue]) {
        CGSize badgeSize = CGSizeZero;
        
        if (NSFoundationVersionNumber > NSFoundationVersionNumber_iOS_6_1) {
            badgeSize = [_badgeValue boundingRectWithSize:CGSizeMake(frameSize.width, 15)
                                                  options:NSStringDrawingUsesLineFragmentOrigin
            ...
            badgeSize = [_badgeValue sizeWithFont:[self badgeTextFont]
                                constrainedToSize:CGSizeMake(frameSize.width, 15)];
#endif
        }
